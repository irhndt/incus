...
function test_add_remove_addresses() {
  ...
  # Add an address using the CLI subcommand "add"
...
function test_nft_block_ping_with_address_set() {
    ...
    incus network address-set create testAS
    incus network address-set add testAS $ip
    ...
    incus network set incusbr0 security.acls="allowping"
    sleep 2
    if ping -c2 "$ip" > /dev/null; then
        success "Ping succeeded despite ACL block."
    else
        error_msg "Ping blocked by ACL."
        incus network set incusbr0 security.acls=""
        incus network acl delete allowping
        incus delete testct --force
        exit 1
    fi
    incus network address-set remove testAS $ip
    if ping -c2 "$ip" > /dev/null; then
        error_msg "Ping to container succeeded."
        incus delete testct --force
        exit 1
    else
        success "Ping to container failed."
    fi
    incus network set incusbr0 security.acls=""
    incus network acl delete allowping
    incus network address-set delete testAS
...
function test_nft_block_tcp_mixed_set(){
    ...
    incus network address-set create testAS
    incus network address-set add testAS "$ip"
    incus network address-set add testAS "$ip6"
    incus network acl create blocktcp5355
    incus network acl rule add blocktcp5355 ingress action=drop protocol=tcp destination_port="5355" destination="\$testAS"
    incus network set incusbr0 security.acls="blocktcp5355"
    if nc -z -w 5 "$ip" 5355; then
...
function test_nft_dynamic_ping_block() {
    ...
    incus network address-set create testAS
    incus network address-set add testAS "$ip_testct"
    incus network address-set add testAS "$ip_testct2"
    ...
    if ping -c2 "$ip_testct" > /dev/null; then
        success "Ping to testct succeeded."
    else
        error_msg "Ping to testct blocked."
        incus network acl delete allowping
        incus network address-set delete testAS
        exit 1
    fi
    ...
    incus network address-set remove testAS "$ip_testct2"
    sleep 1
    ...
    if ping -c2 "$ip_testct2" > /dev/null; then
        error_msg "Ping to testct2 succeeded after address set update."
        incus network acl delete allowping
        incus network address-set delete testAS
        exit 1
    else
        success "Ping to testct2 failed despite being removed from the blocked set."
    fi
    incus network set incusbr0 security.acls=""
    incus network acl delete allowping
    incus network address-set delete testAS
...
function test_load_via_network_classic_nft(){
  # Create as
  incus network address-set create AS1
  incus network address-set add AS1 1.1.1.1
  # Create ACL
  # Create ACL ruel using as
  incus network acl create acl1
  incus network acl rule add acl1 ingress action=allow source='$AS1'
  # Use ACL in network incusbr0
  incus network set $PARENT_NETWORK security.acls=acl1
  if nft -j list sets inet | grep -q AS1_ipv4; then
    success "Address set 'AS1' correctly created in nft"
  else
    error_msg "Address set 'AS1' not found in nft -j list sets inet."
    incus network set $PARENT_NETWORK security.acls=
    incus network acl delete acl1
    incus network address-set delete AS1
    exit 1
  fi
  if nft -j list sets inet | grep -q AS1_ipv6; then
    success "Address set 'AS1' correctly created in nft"
  else
    error_msg "Address set 'AS1' not found in nft -j list sets inet."
    incus network set $PARENT_NETWORK security.acls=
  ...
  incus network acl delete acl1
  incus network address-set delete AS1
  if nft -j list sets inet | grep -q AS1_ip; then
    error_msg "Address set 'AS1' was found in nft should have been gone"
    exit 1
  else
    success "Address set 'AS1' not found in nft -j list sets inet."
  fi
}

function test_load_via_network_nic_classic_nft(){
  # Create as
  incus network address-set create AS1
  incus network address-set add AS1 1.1.1.1
  # Create ACL
  # Create ACL rule using as
  incus network acl create acl1
  incus network acl rule add acl1 ingress action=allow source='$AS1'
  # Use ACL in instance bridged nic
  incus config device set testct eth0 security.acls=acl1
  if nft -j list sets bridge | grep -q AS1_ipv4; then
    success "Address set 'AS1' correctly created in nft"
  else
    error_msg "Address set 'AS1' not found in nft -j list sets inet."
    incus config device set testct eth0 security.acls=
    incus network acl delete acl1
    incus network address-set delete AS1
    exit 1
  fi
  if nft -j list sets bridge | grep -q AS1_ipv6; then
    success "Address set 'AS1' correctly created in nft"
  else
    error_msg "Address set 'AS1' not found in nft -j list sets inet."
    incus config device set testct eth0 security.acls=
  ...
  incus network acl delete acl1
  incus network address-set delete AS1
  if nft -j list sets bridge | grep -q AS1_ip; then
    error_msg "Address set 'AS1' was found in nft should have been gone"
    exit 1
  else
    success "Address set 'AS1' not found in nft -j list sets bridge."
  fi
}

function test_load_via_network_classic_ovn(){
  # Create as
  # Create ACL
  # Create ACL ruel using as
  # Use ACL in network ovntest
  info "Test 28 begins"
  incus network address-set create AS1
  incus network address-set add AS1 1.1.1.1
  ...
  incus network set ovntest security.acls=acl1

  if ovn-nbctl list address_set | grep -q incus_set; then
    success "Address set 'AS1' correctly loaded in OVN"
  else
    error_msg "Address set 'AS1' not found in OVN."
    exit 1
  fi
  ...
  incus network address-set delete AS1
  if ovn-nbctl list address_set | grep -q incus_set; then
    error_msg "Address set 'AS1' was found in ovn nb should have been gone"
    exit 1
  else
    success "Address set 'AS1' not found in ovn nb."
  fi
}

function test_load_via_network_nic_classic_ovn(){
  # Create as
  # Create ACL
  # Create ACL ruel using as
  # Use ACL in instance ovn nic
  incus network address-set create AS1
  incus network address-set add AS1 1.1.1.1

  incus network acl create acl1
  incus network acl rule add acl1 ingress action=allow source='$AS1'
  incus config device set testct eth0 network=ovntest
  incus config device set testct eth0 security.acls=acl1

  if ovn-nbctl list address_set | grep -q incus_set; then
    success "Address set 'AS1' correctly loaded in OVN NIC"
  else
    error_msg "Address set 'AS1' not found in OVN NIC."
    exit 1
  fi
...
function test_load_via_acl_update_nft(){
  # Create as
  # Create ACL empty
  # Use ACL in network incusbr0
  # Create ACL ruel using as
  ...
  incus network set incusbr0 security.acls=acl1
  incus network acl rule add acl1 ingress action=allow source='$AS1'

  
  if nft -j list sets inet | grep -q AS1_ipv4; then
    success "ACL update correctly loaded address set in nft"
  else
    error_msg "ACL update failed to load address set in nft."
    exit 1
  fi
    if nft -j list sets inet | grep -q AS1_ipv6; then
    success "ACL update correctly loaded address set in nft"
...
function test_load_via_acl_update_nic_nft(){
  # Create as
  # Create ACL empty
  # Use ACL in instance bridged nic 
  # Create ACL ruel using as
  ...
  incus network address-set add AS1 1.1.1.1
  incus network acl create acl1
  incus config device set testct eth0 network=$PARENT_NETWORK
  incus config device set testct eth0 security.acls=acl1
  incus network acl rule add acl1 ingress action=allow source='$AS1'
...
function test_load_via_acl_update_ovn(){
  # Create as
  # Create ACL empty
  # Use ACL in network ovntest
  # Create ACL ruel using as
  ...
  if ovn-nbctl list address_set | grep -q incus_set; then
    success "ACL update correctly loaded address set in OVN"
  else
    error_msg "ACL update failed to load address set in OVN."
    exit 1
  fi
...
function test_load_via_acl_update_nic_ovn(){
  # Create as
  # Create ACL empty
  # Use ACL in instance ovn nic 
  # Create ACL ruel using as
  ...
  if ovn-nbctl list address_set | grep -q incus_set; then
    success "ACL update correctly loaded address set in OVN NIC"
  else
    error_msg "ACL update failed to load address set in OVN NIC."
    exit 1
  fi
...
function test_load_via_as_update_nft(){
  # Create as
  # Create ACL empty
  # Use ACL in network incusbr0
  # Create ACL ruel using as
  # Add addresses in AS
  # Check update
  info "Test: Address set update propagates to nft (network scoped ACL)"
  incus network address-set create AS1
  incus network acl create acl1
  incus network set incusbr0 security.acls="acl1"
  incus network acl rule add acl1 ingress action=allow source="\$AS1"
  incus network address-set add AS1 192.0.2.1

  if nft -j list sets inet | grep -q "192.0.2.1"; then
    success "Address set update correctly propagated to nft (network ACL)."
  else
    error_msg "Address set update NOT propagated to nft (network ACL)."
    exit 1
  fi
...
function test_load_via_as_update_nic_nft(){
  # Create as
  # Create ACL empty
  # Use ACL in instance bridged nic 
  # Create ACL ruel using as
  # Add addresses in AS
  # Check update
  info "Test: Address set update propagates to nft (instance NIC scoped ACL)"
  incus network address-set create AS1
  incus network acl create acl1
  incus config device set testct eth0 network=$PARENT_NETWORK
  incus config device set testct eth0 security.acls="acl1"
  incus network acl rule add acl1 ingress action=allow source="\$AS1"
  incus network address-set add AS1 198.51.100.1

  if nft -j list sets bridge | grep -q "198.51.100.1"; then
    success "Address set update correctly propagated to nft (NIC ACL)."
  else
    error_msg "Address set update NOT propagated to nft (NIC ACL)."
    exit 1
  fi
...
function test_load_via_as_update_ovn(){
  # Create as
  # Create ACL empty
  # Use ACL in network ovntest
  # Create ACL ruel using as
  # Add addresses in AS
  # Check update
  info "Test: Address set update propagates to OVN (network scoped ACL)"
  incus network address-set create AS1
  incus network acl create acl1
  incus network set ovntest security.acls="acl1"
  incus network acl rule add acl1 ingress action=allow source="\$AS1"
  incus network address-set add AS1 203.0.113.1

  sleep 2  # Allow OVN to update
  if ovn-nbctl list address_set | grep -q "203.0.113.1"; then
    success "Address set update correctly propagated to OVN (network ACL)."
  else
    error_msg "Address set update NOT propagated to OVN (network ACL)."
    exit 1
  fi
...
function test_load_via_as_update_nic_ovn(){
  # Create as
  # Create ACL empty
  # Use ACL in instance ovn nic 
  # Create ACL ruel using as
  # Add addresses in AS
  # Check update
  info "Test: Address set update propagates to OVN (instance NIC scoped ACL)"
  incus network address-set create AS1
  incus network acl create acl1
  incus config device set testct eth0 network=ovntest
  incus config device set testct eth0 security.acls="acl1"
  incus network acl rule add acl1 ingress action=allow source="\$AS1"
  incus network address-set add AS1 203.0.113.2

  sleep 2  # Allow OVN to update
  if ovn-nbctl list address_set | grep -q "203.0.113.2"; then
    success "Address set update correctly propagated to OVN (NIC ACL)."
  else
    error_msg "Address set update NOT propagated to OVN (NIC ACL)."
    exit 1
  fi
...
function test_acl_use_two_sets(){
  # Create as1
  # Create as2
  # Create ACL empty
  # Use ACL in instance ovn nic 
  # Use ACL in instance nftables bridged nic 
  # Create ACL ruel using as,as1
  # Add addresses in AS and AS1
  # Check update in both networks as
  info "Test: ACL using two address sets across OVN and nft networks"
  incus network address-set create AS1
  incus network address-set create AS2
  incus network acl create acl1
  incus network acl rule add acl1 ingress action=allow source="\$AS1,\$AS2"

  incus config device set testct eth0 security.acls="acl1"
  incus init images:debian/12 testct2 -n incusbr0
  incus config device set testct2 eth0 security.acls=acl1 
  sleep 3

  incus network address-set add AS1 198.51.100.2
  incus network address-set add AS2 203.0.113.3


  # Check nft
  if nft -j list sets bridge | grep -q "198.51.100.2"; then
    success "AS1 correctly propagated to nft."
  else
    error_msg "AS1 NOT propagated to nft."
    exit 1
  fi

  if nft -j list sets bridge | grep -q "203.0.113.3"; then
    success "AS1 correctly propagated to nft."
  else
    error_msg "AS1 NOT propagated to nft."
    exit 1
  ...
  # Check OVN
  if ovn-nbctl list address_set | grep -q "198.51.100.2"; then
    success "AS2 correctly propagated to OVN."
  else
    error_msg "AS2 NOT propagated to OVN."
    exit 1
  fi

  if ovn-nbctl list address_set | grep -q "203.0.113.3"; then
    success "AS2 correctly propagated to OVN."
  else
    error_msg "AS2 NOT propagated to OVN."
    exit 1
  ...
  incus network acl delete acl1
  incus network address-set delete AS1
  incus network address-set delete AS2
}

function test_set_used_by_two_acls(){
  # Create as
  # Create ACL1 empty
  # Use ACL1 in instance ovn nic 
  # Create ACL1 ruel using as
  # Create ACL2 empty
  # Use ACL2 in instance nftables bridged nic 
  # Create ACL2 ruel using as
  # Add addresses in AS
  # Check update
  info "Test: Single address set used by two ACLs across OVN and nft networks"
  incus network address-set create AS1

  incus network acl create acl-ovn
  incus network acl create acl-nft
  incus network acl rule add acl-ovn ingress action=allow source="\$AS1"
  incus network acl rule add acl-nft ingress action=allow source="\$AS1"

  incus config device set testct eth0 security.acls="acl-ovn"
  incus init images:debian/12 testct2 -n incusbr0
  incus config device set testct2 eth0 security.acls=acl-nft
  sleep 3

  incus network address-set add AS1 192.0.2.3

  sleep 2

  # Check nft
  if nft -j list sets bridge | grep -q "192.0.2.3"; then
    success "Address set correctly propagated to nft."
  else
    error_msg "Address set NOT propagated to nft."
    exit 1
  fi

  # Check OVN
  if ovn-nbctl list address_set | grep -q "192.0.2.3"; then
    success "Address set correctly propagated to OVN."
  else
    error_msg "Address set NOT propagated to OVN."
    exit 1
  fi
  ...
  incus config device set testct eth0 security.acls=""
  incus delete testct2 --force
  incus network acl delete acl-ovn
  incus network acl delete acl-nft
  incus network address-set delete AS1
}

## -- Clean up previous runs --

incus delete --force testct || true
incus delete --force testct2 || true
incus network rm ovntest || true
incus network set incusbr0 security.acls= || true
incus network acl rm acl1 || true
incus network acl rm allowping || true
incus network acl rm acl-nft || true
incus network acl rm acl-ovn || true
incus network address-set rm AS1 || true
incus network address-set rm testAS || true
incus network address-set rm testAS2 || true
...
test_load_via_network_classic_nft
success "TEST 26 load_via_network_classic_nft OK"
incus config device set testct eth0 network=$PARENT_NETWORK
test_load_via_network_nic_classic_nft
success "TEST 27 load_via_network_nic_classic_nft"
test_load_via_network_classic_ovn
success "TEST 28 load_via_network_classic_ovn"
test_load_via_network_nic_classic_ovn
success "TEST 29 load_via_network_nic_classic_ovn"
test_load_via_acl_update_nft
success "TEST 30 load_via_acl_update_nft"
test_load_via_acl_update_nic_nft
success "TEST 31 load_via_acl_update_nic_nft"
test_load_via_acl_update_ovn
success "TEST 32 load_via_acl_update_ovn"
test_load_via_acl_update_nic_ovn
success "TEST 33 load_via_acl_update_nic_ovn"
test_load_via_as_update_nft
success "TEST 34 load_via_as_update_nft"
test_load_via_as_update_nic_nft
success "TEST 35 load_via_as_update_nic_nft"
test_load_via_as_update_ovn
success "TEST 36 load_via__acl_update_ovn"
test_load_via_as_update_nic_ovn
success "TEST 37 load_via_acl_update_nic_ovn"
test_acl_use_two_sets
success "TEST 38 acl_use_two_sets"
test_set_used_by_two_acls
success "TEST 39 set_used_by_two_acls"
