...
SELECT networks_zones_records.name
  FROM networks_zones_records
  ORDER BY networks_zones_records.name
`)
...
UPDATE networks_zones_records SET name = ? WHERE name = ?
`)
...
func GetNetworkZoneRecord(ctx context.Context, db dbtx, name string) (_ *NetworkZoneRecord, _err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()
...
func GetNetworkZoneRecordNames(ctx context.Context, db dbtx, filters ...NetworkZoneRecordFilter) (_ []string, _err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()
	...
	if len(filters) == 0 {
		sqlStmt, err = Stmt(db, networkZoneRecordNames)
		if err != nil {
			return nil, fmt.Errorf("Failed to get \"networkZoneRecordNames\" prepared statement: %w", err)
		}
	}

	for i, filter := range filters {
		if filter.NetworkZoneID != nil && filter.ID == nil && filter.Name == nil {
			args = append(args, []any{filter.NetworkZoneID}...)
			if len(filters) == 1 {
				sqlStmt, err = Stmt(db, networkZoneRecordNamesByNetworkZoneID)
				if err != nil {
					return nil, fmt.Errorf("Failed to get \"networkZoneRecordNamesByNetworkZoneID\" prepared statement: %w", err)
				}

				break
			}

			query, err := StmtString(networkZoneRecordNamesByNetworkZoneID)
			if err != nil {
				return nil, fmt.Errorf("Failed to get \"networkZoneRecordNames\" prepared statement: %w", err)
			}
...
func NetworkZoneRecordExists(ctx context.Context, db dbtx, name string) (_ bool, _err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()
...
func GetNetworkZoneRecordID(ctx context.Context, db tx, name string) (_ int64, _err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()
...
func RenameNetworkZoneRecord(ctx context.Context, db dbtx, name string, to string) (_err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()
...
func UpdateNetworkZoneRecord(ctx context.Context, db tx, name string, object NetworkZoneRecord) (_err error) {
	defer func() {
		_err = mapErr(_err, "NetworkZoneRecord")
	}()

	id, err := GetNetworkZoneRecordID(ctx, db, name)
	if err != nil {
		return err
	}
